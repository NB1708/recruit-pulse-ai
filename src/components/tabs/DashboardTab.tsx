...
export default function DashboardTab({ masterData, selectionData, eodData, dailyData, onAiAnalyze, aiLoading, aiError }: DashboardTabProps) {
  const [insight, setInsight] = useState('');
  const currentMonth = new Date().toLocaleString('default', { month: 'long' });

  const metrics = useMemo(() => ({
    activePipeline: masterData.filter(r => r.stage === 'Process' || r.stage === 'Feedback Pending').length,
    joinedThisMonth: selectionData.filter(r => r.candidateStatus === 'Joined' && r.month === currentMonth).length,
    stuckCandidates: masterData.filter(r => daysSince(r.date) >= 5 && r.stage !== 'Joined').length,
    backoutRisk: selectionData.filter(r => ['Backout', 'Dropout'].includes(r.candidateStatus)).length,
  }), [masterData, selectionData, currentMonth]);

  const funnel = useMemo(() => {
    const stages = ['Feedback Pending', 'CV Shortlisted', 'Process', 'Offered', 'Joined'];
    const counts = stages.map(s => s === 'Offered' ? selectionData.filter(r => r.candidateStatus === 'OL released').length : masterData.filter(r => r.stage === s).length);
  ...
  const statusBreakdown = useMemo(() => Object.entries(masterData.reduce((acc, r) => ({ ...acc, [r.clientStatus]: (acc[r.clientStatus] || 0) + 1 }), {} as Record<string, number>)).sort((a, b) => b[1] - a[1]), [masterData]);
  ...
  const urgent = useMemo(() => masterData.filter(r => r.stage === 'Feedback Pending' && daysSince(r.date) >= 7).slice(0, 6), [masterData]);
  const sources = useMemo(() => dailyData.reduce((a, r) => ({ ...a, [r.source]: (a[r.source] || 0) + 1 }), {} as Record<string, number>), [dailyData]);
  ...
  return <div className="space-y-6 animate-fade-in">
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <Card className="border-border bg-card"><CardHeader><CardTitle className="text-sm font-display">Urgent Attention Candidates</CardTitle></CardHeader><CardContent className="space-y-2">{urgent.map(c => <div key={c.candidateName} className="rounded border border-rp-orange/40 bg-rp-orange/10 p-2 text-xs"><p>{c.candidateName} · {c.role}</p><p className="text-muted-foreground">{c.organisation} · {daysSince(c.date)} days</p></div>)}</CardContent></Card>
      <Card className="border-border bg-card"><CardHeader><CardTitle className="text-sm font-display">Source Channel Breakdown</CardTitle></CardHeader><CardContent className="space-y-2">{Object.entries(sources).sort((a,b)=>b[1]-a[1]).map(([s,c]) => <div key={s}><div className="flex justify-between text-xs"><span className="text-muted-foreground">{s}</span><span>{c}</span></div><div className="h-1.5 rounded-full bg-secondary"><div className="h-full rounded-full bg-rp-blue" style={{ width: `${(c/Math.max(...Object.values(sources),1))*100}%` }} /></div></div>)}</CardContent></Card>
    </div>
  </div>;
}
